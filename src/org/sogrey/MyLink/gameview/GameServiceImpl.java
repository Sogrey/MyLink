...
{
	// 定义一个Block[][]数组，只提供getter方法
	private Block[][] pieces;
	// 游戏配置对象
	...
	public boolean hasBlocks()
	{
		...
		for (int i = 0; i < pieces.length; i++)
		{
			...
			{
				// 只要任意一个数组元素不为null，也就是还剩有非空的Block对象
				if (pieces[i][j] != null)
				{
					return true;
				}
	...
	// 根据触碰点的位置查找相应的方块
	@Override
	public Block findBlock(float touchX, float touchY)
	{
		// 由于在创建Block对象的时候, 将每个Block的开始座标加了
		// Config中设置的beginImageX/beginImageY值, 因此这里要减去这个值
		...
		// 获取relativeX座标在Block[][]数组中的第一维的索引值
		// 第二个参数为每张图片的宽
		int indexX = getIndex(relativeX, Config.PIECE_WIDTH);
		// 获取relativeY座标在Block[][]数组中的第二维的索引值
		// 第二个参数为每张图片的高
		int indexY = getIndex(relativeY, Config.PIECE_HEIGHT);
	...
	// 工具方法, 根据relative座标计算相对于Block[][]数组的第一维
	...
	// 实现接口的link方法
	@Override
	public LinkPoint link(Block p1, Block p2)
	{
		// 两个Block是同一个, 即选中了同一个方块, 返回null
		if (p1.equals(p2))
			return null;
		...
		{
			// 它们在同一行并可以相连
			if (!isXBlock(p1Point, p2Point, Config.PIECE_WIDTH))
			{
				return new LinkPoint(p1Point, p2Point);
			}
		}
		// 如果两个Block在同一列
		if (p1.getIndexX() == p2.getIndexX())
		{
			if (!isYBlock(p1Point, p2Point, Config.PIECE_HEIGHT))
			{
				// 它们之间没有真接障碍, 没有转折点
				return new LinkPoint(p1Point, p2Point);
			}
		...
		Point cornerPoint = getCornerPoint(p1Point, p2Point,
			Config.PIECE_WIDTH, Config.PIECE_HEIGHT);
		if (cornerPoint != null)
		{
			return new LinkPoint(p1Point, cornerPoint, p2Point);
		}
		// 该map的key存放第一个转折点, value存放第二个转折点,
		// map的size()说明有多少种可以连的方式
		Map<Point, Point> turns = getLinkPoints(p1Point, p2Point,
			Config.PIECE_WIDTH, Config.PIECE_WIDTH);
		if (turns.size() != 0)
		{
	 ...
	 */
	private LinkPoint getShortcut(Point p1, Point p2, Map<Point, Point> turns,
		int shortDistance)
	{
		List<LinkPoint> infos = new ArrayList<LinkPoint>();
		// 遍历结果Map,
		for (Point point1 : turns.keySet())
		{
			Point point2 = turns.get(point1);
			// 将转折点与选择点封装成LinkPoint对象, 放到List集合中
			infos.add(new LinkPoint(p1, point1, point2, p2));
		}
		return getShortcut(infos, shortDistance);
	}
	
	/**
	 * 从infos中获取连接线最短的那个LinkPoint对象
	 * 
	 * @param infos
	 * @return 连接线最短的那个LinkPoint对象
	 */
	private LinkPoint getShortcut(List<LinkPoint> infos, int shortDistance)
	{
		int temp1 = 0;
		LinkPoint result = null;
		for (int i = 0; i < infos.size(); i++)
		{
			LinkPoint info = infos.get(i);
			// 计算出几个点的总距离
	 ...
	 * @param p2
	 * @param pieceWidth
	 * @return 两个Block之间有障碍返回true，否则返回false
	 */
